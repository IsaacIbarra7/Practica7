...
def autenticar_Usuario(estudiantes, contrasena):
    res = {}
    ...
    if estudiantes != " ":
        ...
        for alumno in alumnos:
            datos = alumno.split("|")
            if len(datos) > 0:
                if estudiantes == datos[0]:
                    cont_usuario = contrasena.encode("utf-8")
                    cont_encriptada = datos[3].encode("utf-8")
                    coincide = bcrypt.checkpw(cont_usuario, cont_encriptada)
                    if coincide == True:
                        res = {"Bandera": True, "Usuario ": datos[1],
                                     "Mensaje: ": "********** Bienvenido al Sistema de Autenticación de Usuarios **********"}
                    else:
                        res = {"Bandera ": False, "Usuario ": datos[1],
                                     "Mensaje: ": "Contraseña Incorrecta..."}
                    break
    if len(res) == 0:
...
autenticar_Usuario("18420484", "0609x5r_$")
